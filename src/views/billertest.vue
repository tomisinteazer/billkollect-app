        ...
        <div class="  p-6 bg-white  rounded-b-2xl">
            ...
            <div v-if="selectedProduct && selectedProduct.metadata && selectedProduct.metadata.customFields"
                ...
                <form @submit.prevent="submitForm">
                    <div class="space-y-6">
                        ...
                        <div v-if="!selectedProduct.isAmountFixed" class="grid gap-2">
                            ...
                            <input id="amount" type="number" v-model="formData.amount" required
                                :disabled="isMultiSelectWithPrice" class="p-2 border border-zinc-300 rounded-md"
                        ...
                        <div v-for="field in sortedCustomFields" :key="field.variableId" class="grid gap-2">
                            ...
                            <multiSelect v-else-if="field.type === 'multiselect'" :field="field"
                                @update:selected="handleMultiSelect(field.variable_name, $event)" />





                            <multiSelectWithPrice v-else-if="field.type === 'multiselectwithprice'" :field="field"
                                @update:selected="handleMultiSelectWithPrice(field.variable_name, $event)" />
                    ...
                    <div class="mt-6">
                        <button type="submit" @click="isubmitForm()"
                            class="py-2 px-4 bg-clifford text-white rounded-md hover:bg-hard">
                            Continue
                        </button>
                    </div>
                </form>
...
import MultiSelectWithPrice from '@/components/MultiSelectWithPriceCopy.vue'
...
export default {
    ...
    computed: {
        ...
        isMultiSelectWithPrice() {
            return this.selectedProduct && this.selectedProduct.metadata && this.selectedProduct.metadata.customFields
                ? this.selectedProduct.metadata.customFields.some(field => field.type === 'multiselectwithprice')
                : false;
        },
    ...
    methods: {
        handleMultiSelect(fieldName, selectedCodes) {
            this.formData[fieldName] = selectedCodes.join(',');
        },
        ...
        handleMultiSelectWithPrice(fieldName, { codes, amount }) {
            this.formData[fieldName] = codes.join(',') // Or keep as array depending on backend expectations
            this.formData.amount = amount // Overwrite total amount if needed
        },
        ...
        isubmitForm() {
            ...
            const payload = {
                ...
                amount: parseFloat(this.formData.amount) || this.selectedProduct.amount || 0,
                customFields: {}
            };
